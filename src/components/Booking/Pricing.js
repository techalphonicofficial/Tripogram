...
<<<<<<< HEAD
    ...
    try {
      const personalDetailsValue = localStorage.getItem("personalDetails");
      const personalDetails = personalDetailsValue
        ? JSON.parse(decrypt(personalDetailsValue))
        : {};
      return { ...personalDetails, ...bookingData };
    } catch {
      return bookingData;
    }
=======
...
<<<<<<< HEAD
        const missingBookingFields = [
          ...
          ["phone", createBookingPayload.phone],
          ["start_date", createBookingPayload.start_date],
          ["package_id", createBookingPayload.package_id],
          ["payment_type", createBookingPayload.payment_type],
          ["final_amount", createBookingPayload.final_amount],
        ]
...
<<<<<<< HEAD
      ...
      const isCancelled = Object.keys(paymentError).length === 0
        ...
        || /cancel/i.test(paymentError.description || "");
      if (isCancelled) {
        console.info("Payment cancelled by the customer.");
      } else {
